...
CREATE TABLE IF NOT EXISTS `mydb`.`Metric` (
  ...
  `fk_CcdToMetricId` INT UNSIGNED NOT NULL,
  ...
  INDEX `fk_metric_idx` (`fk_CcdToMetricId` ASC),
  CONSTRAINT `fk_metric`
    FOREIGN KEY (`fk_CcdToMetricId`)
    REFERENCES `mydb`.`CcdToMetric` (`pk_ccdToMetricId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
